...
const App = {
    ...
    init: async function() {
        ...
        // console.log(liquidity0, liquidity1, this.accounts);
    ...
    openPosition: async function() {
        ...
        let token_addr = this.meta[sAsset]._address;
        await this.meta['Mint'].methods.openPosition(deposit, token_addr, CR).send({from: this.accounts[0]});
        this.refreshBalance();
        this.setStatus("Transaction complete!");
    },
    ...
    addLiquidity: async function() {
        ...
        await this.approve('sBNB', this.meta['Swap'].options.address, liquidity0);
        await this.approve('sTSLA', this.meta['Swap'].options.address, liquidity1);
        // no need for addLiquidity(liquidity1)
